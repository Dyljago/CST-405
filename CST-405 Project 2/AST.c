...
void traverseAST(ASTNode* node, int level) {
    ...
    typedef struct ASTNode {
        ...
        case NodeType_OtherStmt:
            printIndent(level);
            printf("Stmt: %s ", node->otherStmt.varName);
            traverseAST(node->otherStmt.expr, level + 1);
            break;
        case NodeType_BinOp:
            printIndent(level);
            printf("BinOp: %c\n", node->binOp.operator);
...
void freeAST(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case NodeType_AssignStmt:
            ...
            break;
        case NodeType_OtherStmt:
            free(node->otherStmt.varName);
            free(node->otherStmt.expr);
            break;
        case NodeType_BinOp:
...
ASTNode* createNode(NodeType type) {
    ...
    switch (type) {
        ...
        case NodeType_AssignStmt:
            ...
            break;
        case NodeType_OtherStmt:
            newNode->otherStmt.varName = NULL;
            newNode->otherStmt.expr = NULL;
            break;
        case NodeType_BinOp:
