...
void addSymbol(SymbolTable* table, char* name, char* type, char* kind, int size, char* scope) {
...
void printSymbolTable(SymbolTable* table) {
    ...
    for (int i = 0; i < table->size; i++) {
        ...
        while (symbol != 0) {
            printf("Name: %s, \tType: %s, \tKind: %s, \tSize: %d, \tScope: %s\n", symbol->name, symbol->type, symbol->kind, symbol->size, symbol->scope);
            symbol = symbol->next;
        }
    }
    printf("------------------------\n");
