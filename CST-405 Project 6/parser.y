...
%type <ast> Program VarDecl VarDeclList Stmt StmtList Expr FuncDecl FuncCall ParamList ParamDecl WhileStatement IfStatements ElseIfStatements ElseStatement MainFunc FuncDeclList Body Array Array_Decl BinOp ExprList
%start Program
...
WhileStatement: WHILE LPAR Expr RPAR LBRA Body RBRA {
	printf("PARSER: Recognized while statement\n"); 
	printf("\n******************************************\n\n"); 
	$$ = malloc(sizeof(ASTNode));
	$$->type = NodeType_WhileStmt;
	$$->whileStatement.expr = $3;
	$$->whileStatement.body = $6;
}
